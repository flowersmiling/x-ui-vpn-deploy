...
**结论**：CF 的真正价值不是"加速"，是"备胎"——在 VPS IP 被封时让客户端仍能连上。日常体验直连远胜走 CF，尤其是对延迟敏感、有严格超时限制的场景（比如某些机构的 SSO/联合登录流程，实战中就遇到过直连秒开、走 CF 反而因为多一跳延迟导致登录页超时的情况）。
...
> ⚠️ **如果部署时 `ROOT_DOMAIN` 实际就是用户完整拥有的子域名本身**（比如根域是公共后缀 PSL、真正管理的是 `myname.cc.cd` 这种情况，见 `manual-deploy.md` 步骤 0 的域名核实说明），就没有一个"更上层的根域"可以拿来当灰云直连入口了——这种情况下不新建额外 DNS 记录，直接让客户端把"地址"填成服务器真实 IP、"SNI/Host"保持域名不变即可（见下方"客户端配置模式"），效果是一样的，只是不需要在 CF 面板另外建一条记录。
...
- WS/XHTTP Upgrade 时 Host 头正确（nginx location 路由生效）

对应的 VLESS 链接直接把 `@` 后面的地址换成服务器 IP，`sni`/`host` 参数保持域名不变：

```
vless://{UUID}@{SERVER_IP}:443?encryption=none&security=tls&sni={DOMAIN}&type=xhttp&host={DOMAIN}&path=%2F{WS_PATH}&fp=chrome#Direct-NoCF
```
...
| 三条全断 + 伪装站也打不开 | VPS 真挂或被 ban，去 VPS 控制台看（也可能是内存/资源耗尽整机失联，见 `troubleshooting.md` 对应章节） |
| 某个特定网站（尤其登录/SSO 类）反复超时，其他都正常 | 大概率不是线路问题，是客户端系统代理模式漏了 UDP/DNS，见 `troubleshooting.md` |
